#!/usr/bin/env python
import roslib
roslib.load_manifest('tcp_to_arduino')
import rospy
from tcp_to_arduino.srv import *
import time

class Arduino_Data_Client:
    def __init__ (self, rate, action="send"):
        rospy.init_node('send_data_packet_to_arduino_client', anonymous=True) 
        rospy.wait_for_service('send_data_packet_to_arduino')
        self.send_data_packet_to_arduino = rospy.ServiceProxy('send_data_packet_to_arduino', SendDataPacketToArduino)
        self.prev_msg = 'set'
        self.action = action
        
        self.main(rate)
                
    def main(self, rate):
        try:
            if action == "send":
                rospy.Timer( rospy.Duration(1/float(rate)), self.send_data )
            elif action == "recieve":
                rospy.Timer( rospy.Duration(1/float(rate)), self.recieve_data )
            rospy.spin()
        except KeyboardInterrupt:
            print "Shutting Down"
        
    def send_data(self, timer):
        if self.prev_msg == 'set':
            data = self.send_data_to_arduino("get", "sensor1", 0)
            self.prev_msg = 'get'
            print "received data: ", data
        elif self.prev_msg == 'get':
            data = self.send_data_to_arduino("set", "motor", 25)
            self.prev_msg = 'set'
            print "set data: ", data   
            
    def recieve_data(self, timer):    
        
        actions = ["g", "g", "g", "g"]
        data_names = ["ax", "ay", "az", "s"]
        data_vals = [0,0,0,0]
         
        time_start = time.time()
        data_arr = self.send_data_packet_to_arduino(actions, data_names, data_vals)
        time_elapsed = time.time() - time_start
        print data_arr, "|| time elapsed: ", time_elapsed

if __name__ == '__main__':

    rate = 10
    action = "recieve"
    arduino_data_transmission = Arduino_Data_Client(rate, action)
